 ...
 * @LastEditors: liuYang
 * @LastEditTime: 2020-06-17 17:01:21
 * @mustParam: 必传参数
 * @optionalParam: 选传参数
 */
...
export const uploadImage = ({
  ...
  return new Promise(resolve => {
    Taro.chooseImage({
      ...
      success: async (res) => {
        ...
        const aliOSSData = await getOSSData({});
        uploadMultipleFiles(tempFilePaths, aliOSSData, that).then(filePathArray => {
  ...
  * @return void
  */
export const uploadMultipleFiles = (filePathsArray, aliOSSData, that) => {
  return new Promise(resolve => {
    let count = 0
